...
const components: Record<string, JsonSchema> = {
  ...
  Issue: {
    ...
    properties: {
      ...
      dueStatus: { type: ["string", "null"], enum: ["overdue", "due-now", null], description: "Schedule state vs current period; null when unplanned or closed" },
    },
    additionalProperties: false,
  },
  RepoFile: {
  ...
  Pull: {
    ...
    properties: {
      ...
      ci: { type: ["string", "null"], enum: ["success", "failure", "pending", null] },
      reviewDecision: { type: ["string", "null"], enum: ["approved", "changes_requested", "commented", null] },
      lastCommitAt: nullableString,
      repo: { type: "string", description: "'' = product repo; else 'owner/name' for a cross-repo PR" },
    },
    additionalProperties: true,
  },
  FlowResult: { type: "object", properties: {
    state: { type: "string" },
    score: { type: "number" },
    signals: { type: "array", items: { type: "string" } },
    boardStatus: { ...nullableString, description: "Raw pinned-project Status label overlaid for display; null off-board" },
    disagreement: { type: ["string", "null"], enum: ["board-done-open", "board-active-merged", "board-review-idle", null], description: "Board-vs-inferred-flow contradiction; display-only and never affects momentum or at-risk" },
    noPrLinked: { type: "boolean", description: "No linked PR; muted pill note" },
  }, additionalProperties: true },
